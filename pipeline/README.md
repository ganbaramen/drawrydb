...
setlist's own running order) plus 作詞/作曲/編曲/振付 credits and a
free-form note — e.g. a link to the lyrics post, auto-linked if it's a
bare URL. All optional per song and independent of `slug`; a song can
have a slug with no credits filled in yet, which is the current state
